...
export default async function (dom, data) {
  ...
  let cloudParamCode = setCloudParam({
    param: JSON.stringify({
      path: 'pages/helpSell/grouponShare/index',
      params: { id: grouponId, recommendId: userId },
    }),
  });
  ...
  await (async function () {
    ...
    canvas.ctx.fillText(`单品最多可得${ratio}%佣金`, posterPaddingLeft, 1152);
    ...
    canvas.drawTriangle({ x: 478, y: 1250, height: 18, bgc: '#f5f5f5', direction: 'right' });

    canvas.ctx.font = `28px normal`;
    canvas.ctx.fillStyle = '#999';
    canvas.ctx.fillText(`长按识别小程序 立即帮卖TA的团`, posterPaddingLeft + 20, 1218 + 20);
  })();
