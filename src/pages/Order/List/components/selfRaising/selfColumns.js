...
export const columns = (btnGroup, checkDetail) => {
  return [
    ...
    },
    {
      ...
      align: 'center',
      render: ({ orderPickup }) => {
        const data = orderPickup;
        if (!data) return '--';
        return (
          <>
            <Collapse bordered={false} ghost>
              <Panel header={`自提点:${data.pickupName}`} key="1">
                <p className="pl24">自提点地址:{setAddress(data, 'pickup')}</p>
                <p className="pl24">自提点电话:{data.pickupMobile || '--'}</p>
                <p className="pl24">自提点联系人:{data.pickupContact || '--'}</p>
                <p className="pl24">取货人姓名:{data.username || '--'}</p>
                <p className="pl24">取货人电话:{data.mobile || '--'}</p>
              </Panel>
            </Collapse>
          </>
        );
